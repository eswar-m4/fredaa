...
function RefreshPage() {
  ...
  const [backendEstimate, setBackendEstimate] = useState<{ setupDays: number; firstRunHrs: number; monthlyRecords: number } | null>(null);

  useEffect(() => {
    setBackendEstimate(null);
  }, [draft.name, draft.urls, draft.fields, draft.frequency, draft.format, draft.owner, draft.notes]);
  ...
  async function submitProject() {
    ...
    try {
      const urls = draft.urls.filter((u) => u.trim());
      const response = await apiFetch("/api/v1/source-analysis/analyze", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          format: draft.format,
          owner: draft.owner || "name@company.com",
          notes: draft.notes,
        }),
      });
      ...
      const setupDays = data.setup_days;
      const firstRunHrs = data.first_run_hours;
      const monthlyRecords = data.monthly_records;

      setBackendEstimate({ setupDays, firstRunHrs, monthlyRecords });
      ...
      setNotice(
        `Project “${draft.name}” submitted as ${nextReqId()} — estimate: ${setupDays} days setup, ~${fmt(monthlyRecords)} records/month. Admin notified.Status will update in database.`,
      );
              ...
              <div className="space-y-3 flex flex-col">
                ...
                <div className="mt-auto rounded-lg border border-primary/30 bg-gradient-to-br from-primary/10 to-transparent p-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-2">
                    <Est label="Setup" value={`${backendEstimate ? backendEstimate.setupDays : draftEstimate.setupDays} days`} />
                    <Est label="First run" value={`${backendEstimate ? backendEstimate.firstRunHrs : draftEstimate.firstRunHrs} hrs`} />
                    <Est label="Records / mo" value={fmt(backendEstimate ? backendEstimate.monthlyRecords : draftEstimate.monthlyRecords)} />
                  </div>
                  <Button size="sm" className="w-full mt-3" onClick={submitProject} disabled={!draft.name.trim() || isEstimating}>
                    <FolderPlus className="h-3.5 w-3.5" /> {isEstimating ? "Analyzing site..." : "Submit for estimate"}
                  </Button>
