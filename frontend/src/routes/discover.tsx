...
const OBJECTIVE =
  "Ask Freda - your AI solution consultant for company (firmographic) data.\n\nTell me what you need in your own words - for example \"phone and address for hospitals in Chennai, refreshed monthly\". I'll only ask for what's still missing.";
...
function FredaAi() {
  ...
  function handleIntake(text: string) {
    ...
    if (match) {
      say(
        `We already have a "${match.name}" solution in place under ${match.category} - it covers ${match.tagline.toLowerCase()}. You can start there from Solutions, or carry on here and I'll tailor a new solution to your exact scope.`,
      );
    ...
    if (intent.captured.length) {
      say(
        ["Got it. From your request I already have:", ...intent.captured.map((c) => `• ${c.label}: ${c.value}`), "", "Just a few gaps left."].join(
          "\n",
        ),
      );
    } else {
