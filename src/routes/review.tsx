...
import { buildReviewSummary, type ReviewSummary } from "@/lib/review-summary";

export const Route = createFileRoute("/review")({
  head: () => ({ meta: [{ title: "Review Queue - Freda" }] }),
  component: Review,
});
...
function Review() {
  ...
  return (
    <AppLayout>
      <PageHeader
        title="Review Queue"
        subtitle="Solution jobs use sampling + A/D/M/V change tracking. Agent jobs are reviewed per source (full dump on schedule, or change monitoring with highlighted deltas)."
      />
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(["All", "By Dataset", "By Source"] as const).map((m) => (
              ...
              >
                {m === "By Dataset" ? "Solutions" : m === "By Source" ? "Agents" : m} {m !== "All" && <span className="opacity-70">({allJobs.filter((j) => j.mode === m).length})</span>}
              </button>
            ))}
