import { createFileRoute, useNavigate } from "@tanstack/react-router";
import { useEffect, useMemo, useRef, useState } from "react";
import {
  ArrowRight,
  Bot,
  Boxes,
  CheckCircle2,
  Circle,
  Clock,
  Database,
  Download,
  FileInput,
  Filter,
  Globe,
  Layers,
  Send,
  ShieldCheck,
  Sparkles,
  User as UserIcon,
} from "lucide-react";
import { AppLayout } from "@/components/AppLayout";
import { Badge, Button, Card, PageHeader, SectionTitle } from "@/components/ui-bits";
import {
  buildProposal,
  buildQuestions,
  isOther,
  optionLabel,
  otherText,
  OTHER_PREFIX,
  SECTORS,
  type Answers,
  type Proposal,
  type Question,
  type Focus,
  type SectorId,
  type WorkflowNode,
} from "@/lib/freda-firmographic";
import { setUseCase } from "@/lib/useCase";
import { fallbackReply, readIntent, type Family } from "@/lib/freda-intent";
import { buildProductProposal, buildProductQuestions } from "@/lib/freda-product";
import { assessAgentMatch, matchSolution, type SolutionMatch } from "@/lib/freda-match";

export const Route = createFileRoute("/discover")({
  head: () => ({
    meta: [
      { title: "Ask Freda - AI solution consultant" },
      {
        name: "description",
        content:
          "A focused chat that gathers your firmographic data requirement, then proposes sources, attributes, workflow, volume and timeline.",
      },
      { property: "og:title", content: "Ask Freda - AI solution consultant" },
      {
        property: "og:description",
        content: "Requirement-gathering chat for company-level datasets: sources, attributes, workflow, volume, timeline.",
      },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: FredaAi,
});

type Turn =
  | { role: "freda"; kind: "text"; text: string; note?: string }
  | { role: "user"; kind: "text"; text: string }
  | {
      role: "freda";
      kind: "links";
      text: string;
      links: { label: string; to: string; useCase?: "targeted" | "openweb"; datasetId?: string }[];
    }
  | { role: "freda"; kind: "question"; q: Question };

const OBJECTIVE =
  "Ask Freda — your AI solution consultant for company (firmographic) data.\n\nTell me what you need in your own words and I'll ask only what's still missing.";

function describeDataset(d: SolutionMatch["dataset"]) {
  const attrs = d.outputAttributes.slice(0, 5).map((a) => a.label).join(", ");
  const more = d.outputAttributes.length > 5 ? ` +${d.outputAttributes.length - 5} more` : "";
  const src = `${d.sources.length} wired source${d.sources.length === 1 ? "" : "s"} (company website first)`;
  return `${attrs}${more} across ${src}`;
}

type Phase = "intake" | "interview" | "confirm" | "done" | "recommend";



function FredaAi() {
  const navigate = useNavigate();
  const scrollRef = useRef<HTMLDivElement>(null);

  const [answers, setAnswers] = useState<Answers>({});
  const [currentId, setCurrentId] = useState<string | null>(null);
  const [phase, setPhase] = useState<Phase>("intake");
  const [otherDrafts, setOtherDrafts] = useState<Record<string, string>>({});
  const [proposal, setProposal] = useState<Proposal | null>(null);
  const [submitted, setSubmitted] = useState(false);
  const [request, setRequest] = useState("");
  const [focus, setFocus] = useState<Focus[]>([]);
  const [family, setFamily] = useState<Family>("firmographic");
  const [pending, setPending] = useState<{ answers: Answers; focus: Focus[]; family: Family } | null>(null);
  const [closest, setClosest] = useState<SolutionMatch | null>(null);
  const [dropped, setDropped] = useState<string[]>([]);
  const [draft, setDraft] = useState("");
  const [turns, setTurns] = useState<Turn[]>([
    { role: "freda", kind: "text", text: OBJECTIVE, note: "Public sources only. Estimates, not a quote." },
  ]);

  /** questions rebuilt for a specific answer set - avoids stale closures */
  const questionsFor = (a: Answers, f: Focus[] = focus, fam: Family = family) => {
    if (fam === "product") return buildProductQuestions(a);
    const sid = (a["sector"]?.[0] ?? null) as SectorId | null;
    return buildQuestions(sid && sid in SECTORS ? sid : null, { focus: f, answers: a });
  };

  const questions = useMemo(() => questionsFor(answers, focus, family), [answers, focus, family]);
  const current = questions.find((q) => q.id === currentId);

  const makeProposal = (a: Answers, f: Focus[], fam: Family) =>
    fam === "product" ? buildProductProposal(a, request) : buildProposal(a, request, f);


  useEffect(() => {
    scrollRef.current?.scrollTo({ top: scrollRef.current.scrollHeight, behavior: "smooth" });
  }, [turns, currentId, proposal]);

  const say = (text: string, note?: string) => setTurns((p) => [...p, { role: "freda", kind: "text", text, note }]);
  const sayLinks = (text: string, links: Extract<Turn, { kind: "links" }>["links"]) =>
    setTurns((p) => [...p, { role: "freda", kind: "links", text, links }]);
  const nextUnanswered = (a: Answers, f?: Focus[], fam?: Family) =>
    questionsFor(a, f, fam).find((q) => !(a[q.id] ?? []).length) ?? null;

  function summarise(a: Answers, f: Focus[] = focus, fam: Family = family) {
    const p = makeProposal(a, f, fam);
    return [
      "Here's the requirement as I've captured it:",
      ...(request ? [`• Your request: ${request}`] : []),
      ...p.metadata.slice(0, 8).map((m) => `• ${m.label}: ${m.value}`),
      "",
      "If that reads right, confirm and I'll lay out the process, sources, volume and timeline. Anything to add or reword, just tell me.",
    ].join("\n");
  }

  function finalSummary(p: Proposal) {
    return [
      "Requirement summary",
      `• Solution: ${p.title}`,
      ...p.metadata.slice(0, 8).map((m) => `• ${m.label}: ${m.value}`),
      `• Attributes: ${p.attributes.slice(0, 8).join(", ")}${p.attributes.length > 8 ? ` +${p.attributes.length - 8} more` : ""}`,
      "",
      ...(closest
        ? [
            "",
            `Closest existing solution: "${closest.dataset.name}" (${closest.dataset.category}) — it already carries ${describeDataset(closest.dataset)}.`,
            closest.gaps.length ? `It stops short of ${closest.gaps.join(", ")}.` : "",
            "Open it below if that's close enough, or carry on as a customised solution — volume, sources and timeline are on the right.",
          ].filter(Boolean)
        : ["", "Nothing in the catalogue covers this as-is, so it'll be raised as a new solution. Sources, volume and timeline are on the right — submit when it looks right."]),
    ].join("\n");
  }

  function goToNext(a: Answers, f?: Focus[], fam?: Family) {
    const nxt = nextUnanswered(a, f, fam);
    if (!nxt) {
      setCurrentId(null);
      setPhase("confirm");
      say(summarise(a, f, fam ?? family));
    } else {
      setCurrentId(nxt.id);
      // Attribute pickers start fully selected - the user unselects what they don't need.
      if (nxt.multi && (nxt.id === "core" || nxt.id === "pfields")) {
        const all = nxt.options.map((o) => o.id);
        setAnswers((prev) => ({ ...prev, [nxt.id]: all }));
      }
      setTurns((p) => [...p, { role: "freda", kind: "question", q: nxt }]);
    }
  }

  function choose(q: Question, optId: string) {
    setAnswers((prev) => {
      const cur = prev[q.id] ?? [];
      if (!q.multi) return { ...prev, [q.id]: [optId] };
      return { ...prev, [q.id]: cur.includes(optId) ? cur.filter((x) => x !== optId) : [...cur, optId] };
    });
    if (!q.multi) queueMicrotask(() => advance(q, [optId]));
  }

  function setOther(q: Question, text: string) {
    setOtherDrafts((p) => ({ ...p, [q.id]: text }));
    setAnswers((prev) => {
      const rest = (prev[q.id] ?? []).filter((v) => !isOther(v));
      const value = text.trim() ? [OTHER_PREFIX + text.trim()] : [];
      return { ...prev, [q.id]: q.multi ? [...rest, ...value] : value.length ? value : rest };
    });
  }

  function advance(q: Question, forced?: string[]) {
    const chosen = forced ?? answers[q.id] ?? [];
    if (!chosen.length) return;
    const merged = { ...answers, [q.id]: chosen };
    setAnswers(merged);
    setTurns((prev) => [...prev, { role: "user", kind: "text", text: chosen.map((id) => optionLabel(q, id)).join(", ") }]);
    goToNext(merged);
  }

  /* ---------------------------- free-text handling ---------------------------- */

  function startInterview(a: Answers, f: Focus[], fam: Family, lead: string) {
    const gapCount = questionsFor(a, f, fam).filter((q) => !(a[q.id] ?? []).length).length;
    if (!gapCount) {
      say(lead);
      setCurrentId(null);
      setPhase("confirm");
      say(summarise(a, f, fam));
      return;
    }
    say(
      `${lead}\n\nLet me understand a little more about what you need — pick an option below, or just answer in your own words. If something doesn't apply, skip it and I'll carry on with a sensible default.`,
    );
    setPhase("interview");
    goToNext(a, f, fam);
  }

  function handleIntake(text: string) {
    const intent = readIntent(text);
    setRequest(text);
    setFocus(intent.focus);
    setFamily(intent.family);
    const merged = { ...answers, ...intent.answers };
    setAnswers(merged);
    setPending({ answers: merged, focus: intent.focus, family: intent.family });

    if (intent.outOfScope) {
      say(
        "Part of that sits outside Freda's scope - we only read public pages, so no logins, paywalls or personal data. I'll scope the public, company-level part of it.",
      );
    }

    // 1. what did I understand?
    if (intent.captured.length) {
      say(["Here's what I picked up from your request:", ...intent.captured.map((c) => `• ${c.label}: ${c.value}`)].join("\n"));
    }

    // 2. existing agents first
    const sector = (merged["sector"]?.[0] ?? null) as SectorId | null;
    const agent = assessAgentMatch(text, {
      agentHits: intent.agentHits.length,
      newSites: intent.newSites.length,
      topN: intent.topN,
    });

    if (agent) {
      const links = intent.agentHits.slice(0, 3).map((h) => ({
        label: `Open ${h.hit.label}`,
        to: h.hit.href,
        useCase: h.hit.where === "Agents" ? ("targeted" as const) : ("openweb" as const),
      }));
      if (agent.coverage === "full") {
        sayLinks(
          [
            "Good news — an agent we've already onboarded covers this, so there's nothing new to build:",
            ...intent.agentHits.map((h) => `• ${h.site} — ${h.hit.label} (${h.hit.where})`),
            "",
            "Open it below and you can run the extraction straight away. If your scope is wider than that, just tell me and we'll keep going.",
          ].join("\n"),
          links,
        );
        setPhase("recommend");
        return;
      }
      sayLinks(
        [
          "A couple of these sites are already live as agents:",
          ...intent.agentHits.map((h) => `• ${h.site} — ${h.hit.label} (${h.hit.where})`),
          "",
          `They cover part of what you're after — ${agent.gaps.join("; ")} — so let's talk through the rest.`,
        ].join("\n"),
        links,
      );
    }

    // 3. then existing solutions, matched on domain - never on a shared keyword
    const match = matchSolution(text, { family: intent.family, sector, focus: intent.focus });
    if (match) {
      const why = match.reasons.length ? ` (${match.reasons.join(", ")})` : "";
      setClosest(match);
      if (match.coverage === "full" && !agent) {
        sayLinks(
          `That maps neatly onto "${match.dataset.name}" under ${match.dataset.category}${why} — ${match.dataset.tagline.toLowerCase()}. It already carries ${describeDataset(match.dataset)}. Open it and configure your run, or stay here and I'll tailor something closer to your scope.`,
          [
            {
              label: `Open ${match.dataset.name} setup`,
              to: "/any-site",
              useCase: "openweb",
              datasetId: match.dataset.id,
            },
          ],
        );
        setPhase("recommend");
        return;
      }
    } else if (!agent) {
      setClosest(null);
      say("I had a look across the agent and solution catalogues and nothing quite matches this yet — happy to shape a new solution with you.");
    }

    if (intent.newSites.length) {
      say(
        `${intent.newSites.join(", ")} ${intent.newSites.length > 1 ? "are" : "is"} not onboarded yet - ${intent.newSites.length > 1 ? "they'll" : "it'll"} be raised as new source${intent.newSites.length > 1 ? "s" : ""} with this request.`,
      );
    }

    startInterview(
      merged,
      intent.focus,
      intent.family,
      intent.family === "product"
        ? "Company-level filters won't help here, so I'll keep to the catalogue side of things."
        : "Thanks — that gives me a good starting point.",
    );
  }



  function confirmRequirement() {
    const p = makeProposal(answers, focus, family);
    setProposal(p);
    setPhase("done");
    if (closest) {
      sayLinks(finalSummary(p), [
        {
          label: `Open ${closest.dataset.name} setup`,
          to: "/any-site",
          useCase: "openweb",
          datasetId: closest.dataset.id,
        },
      ]);
    } else {
      say(finalSummary(p));
    }
  }


  function send() {
    const text = draft.trim();
    if (!text) return;
    setDraft("");
    setTurns((p) => [...p, { role: "user", kind: "text", text }]);

    if (phase === "intake" || phase === "recommend") {
      handleIntake(text);
      return;
    }

    const fb = fallbackReply(text);
    if (fb) {
      say(fb);
      return;
    }

    if (phase === "interview" && current) {
      advance(current, [OTHER_PREFIX + text]);
      return;
    }

    if (phase === "confirm") {
      if (/^(ok|okay|yes|yep|go|confirm|continue|next|proceed|sounds good)\b/i.test(text)) {
        confirmRequirement();
      } else {
        say("Noted - I've added that to the requirement. Hit Confirm & continue whenever you're ready to see the full solution.");
      }
      return;
    }


    say("Noted - I've attached that to the solution request.");
  }

  function restart() {
    setAnswers({});
    setOtherDrafts({});
    setCurrentId(null);
    setPhase("intake");
    setProposal(null);
    setSubmitted(false);
    setRequest("");
    setFocus([]);
    setFamily("firmographic");
    setPending(null);
    setClosest(null);
    setDropped([]);
    setTurns([{ role: "freda", kind: "text", text: OBJECTIVE, note: "Public sources only. Estimates, not a quote." }]);
  }

  function submit() {
    if (!proposal) return;
    setSubmitted(true);
    say("Submitted. The admin team picks it up now - you'll see it under 'Solution development in progress' on the dashboard, and it becomes a job once approved.");
  }

  function goBuild() {
    if (!proposal) return;
    setUseCase("openweb");
    navigate({ to: "/any-site" });
  }

  return (
    <AppLayout>
      <PageHeader
        title="Ask Freda"
        subtitle="Tell Freda what you need — it checks existing agents and solutions first, then scopes only the gaps."
        actions={
          <div className="flex items-center gap-2">
            <Badge tone="purple" className="gap-1.5">
              <Sparkles className="h-3.5 w-3.5" /> Firmographic scope
            </Badge>
            {phase !== "intake" && (
              <Button variant="outline" size="sm" onClick={restart}>
                Start over
              </Button>
            )}
          </div>
        }
      />

      <div
        className={[
          "px-7 pb-8 grid grid-cols-1 gap-5 items-start",
          proposal ? "xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]" : "",
        ].join(" ")}
      >
        {/* ---------------- chat ---------------- */}
        <div className="space-y-4">
          <Card className="overflow-hidden">
            <div className="flex items-center gap-2 border-b border-border px-5 py-3">
              <span className="h-7 w-7 rounded-md bg-purple-bg text-purple-token inline-flex items-center justify-center">
                <Bot className="h-4 w-4" />
              </span>
              <div>
                <div className="text-[13px] font-semibold leading-tight">Freda</div>
                <div className="text-[11px] text-muted-foreground">AI solution consultant</div>
              </div>
              <span className="ml-auto inline-flex items-center gap-1.5 text-[11px] text-muted-foreground">
                <span className="h-1.5 w-1.5 rounded-full bg-success animate-pulse" />
                {proposal ? "Chat complete" : "Chat in progress"}
              </span>
            </div>

            <div
              ref={scrollRef}
              className={["overflow-y-auto px-5 py-4 space-y-4", proposal ? "max-h-[560px] min-h-[420px]" : "max-h-[68vh] min-h-[520px]"].join(" ")}
            >
              {turns.map((t, i) =>
                t.kind === "question" ? (
                  <QuestionBubble
                    key={i}
                    q={t.q}
                    active={phase === "interview" && t.q.id === currentId}
                    answers={answers}
                    otherDraft={otherDrafts[t.q.id] ?? ""}
                    onChoose={choose}
                    onOther={setOther}
                    onConfirm={advance}
                    onSelectAll={(q) => setAnswers((prev) => ({ ...prev, [q.id]: q.options.map((o) => o.id) }))}
                    onClearAll={(q) => setAnswers((prev) => ({ ...prev, [q.id]: [] }))}
                  />
                ) : (
                  <div key={i} className={t.role === "user" ? "flex justify-end" : "flex gap-2.5"}>
                    {t.role === "freda" && (
                      <span className="h-7 w-7 shrink-0 rounded-md bg-secondary inline-flex items-center justify-center">
                        <Bot className="h-3.5 w-3.5 text-muted-foreground" />
                      </span>
                    )}
                    <div
                      className={[
                        "rounded-lg px-3.5 py-2.5 text-[13px] leading-relaxed max-w-[80%] whitespace-pre-line",
                        t.role === "user" ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground",
                      ].join(" ")}
                    >
                      {t.text}
                      {"note" in t && t.note && <div className="text-[11.5px] text-muted-foreground mt-1.5">{t.note}</div>}
                      {t.kind === "links" && (
                        <div className="mt-2.5 flex flex-wrap gap-2">
                          {t.links.map((l) => (
                            <Button
                              key={l.to + l.label}
                              size="sm"
                              variant="outline"
                              onClick={() => {
                                if (l.useCase) setUseCase(l.useCase);
                                navigate(
                                  l.datasetId
                                    ? { to: l.to, search: { dataset: l.datasetId } }
                                    : ({ to: l.to } as never),
                                );
                              }}
                            >
                              {l.label} <ArrowRight className="h-3.5 w-3.5" />
                            </Button>
                          ))}
                        </div>
                      )}
                    </div>
                    {t.role === "user" && (
                      <span className="h-7 w-7 shrink-0 rounded-md bg-primary/10 text-primary inline-flex items-center justify-center ml-2">
                        <UserIcon className="h-3.5 w-3.5" />
                      </span>
                    )}
                  </div>
                ),
              )}
            </div>

            {phase === "recommend" && (
              <div className="border-t border-border px-5 py-3 flex flex-wrap items-center gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => {
                    if (!pending) return;
                    startInterview(pending.answers, pending.focus, pending.family, "Of course — let's shape something around your exact scope.");
                  }}
                >
                  Design a customised solution instead
                </Button>
                <span className="text-[11.5px] text-muted-foreground">Or tell me what the existing one is missing and we'll take it from there.</span>
              </div>
            )}

            {phase === "confirm" && (
              <div className="border-t border-border px-5 py-3 flex flex-wrap items-center gap-2">
                <Button size="sm" onClick={confirmRequirement}>
                  <CheckCircle2 className="h-4 w-4" /> Confirm & continue
                </Button>
                <span className="text-[11.5px] text-muted-foreground">
                  Or type extra attributes or details below and confirm after.
                </span>
              </div>
            )}

            <div className="border-t border-border px-5 py-3 space-y-2">
              <div className="flex items-end gap-2">
                <textarea
                  value={draft}
                  onChange={(e) => setDraft(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && !e.shiftKey) {
                      e.preventDefault();
                      send();
                    }
                  }}
                  rows={1}
                  placeholder={
                    phase === "intake"
                      ? "e.g. phone and address for hospitals in Chennai, refreshed monthly"
                      : phase === "confirm"
                        ? "Add extra attributes or details, then confirm"
                        : "Type your answer, or pick an option above"
                  }
                  className="flex-1 resize-none rounded-md border border-input bg-card px-3 py-2 text-[13px] outline-none focus:ring-2 focus:ring-ring/40 placeholder:text-muted-foreground"
                />
                <Button disabled={!draft.trim()} onClick={send}>
                  <Send className="h-4 w-4" /> Send
                </Button>
              </div>
              <div className="text-[11.5px] text-muted-foreground flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5 text-success" />
                Public sources only. Freda asks only what's still missing to scope the build.
              </div>
            </div>
          </Card>

        </div>


        {/* ---------------- proposal ---------------- */}
        {proposal && (
          <div className="space-y-4 xl:sticky xl:top-4 xl:max-h-[calc(100vh-6rem)] xl:overflow-y-auto xl:pr-1">
            <Card className="p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">
                    Proposed solution
                  </div>
                  <div className="text-[16px] font-semibold mt-0.5">{proposal.title}</div>
                  <div className="text-[12px] text-muted-foreground mt-1">
                    Lands in {proposal.routeLabel} · refresh {proposal.cadence.toLowerCase()}
                  </div>
                </div>
                <Badge tone={submitted ? "success" : "warning"}>{submitted ? "Submitted" : "Draft"}</Badge>
              </div>
              <div className="grid grid-cols-2 gap-3 mt-4">
                <Stat icon={Database} label="Estimated volume" value={proposal.volume} sub={proposal.volumeNote} />
                <Stat icon={Clock} label="Estimated timeline" value={proposal.timeline} sub={proposal.validation} />
              </div>
              <div className="flex flex-wrap gap-2 mt-4">
                <Button onClick={submit} disabled={submitted}>
                  <CheckCircle2 className="h-4 w-4" /> {submitted ? "Solution request submitted" : "Submit to create a solution"}
                </Button>
                <Button variant="outline" onClick={goBuild}>
                  Continue in {proposal.routeLabel} <ArrowRight className="h-4 w-4" />
                </Button>
              </div>
            </Card>

            <Card className="p-5">
              <SectionTitle hint={`${proposal.workflow.length} steps`}>Proposed workflow</SectionTitle>
              <WorkflowDiagram nodes={proposal.workflow} />
            </Card>

            <Card className="p-5">
              <SectionTitle hint={`${proposal.attributes.length - dropped.length} of ${proposal.attributes.length} selected`}>
                Data attributes
              </SectionTitle>
              <div className="flex items-center gap-2 mb-2">
                <Button size="sm" variant="outline" onClick={() => setDropped([])} disabled={!dropped.length}>
                  Select all
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setDropped(proposal.attributes)}
                  disabled={dropped.length === proposal.attributes.length}
                >
                  Clear all
                </Button>
                <span className="text-[11.5px] text-muted-foreground">Everything is selected — unselect what you don't need.</span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {proposal.attributes.map((a) => {
                  const on = !dropped.includes(a);
                  return (
                    <button
                      key={a}
                      onClick={() => setDropped((d) => (d.includes(a) ? d.filter((x) => x !== a) : [...d, a]))}
                      className={[
                        "inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-[12px] transition",
                        on
                          ? "border-primary bg-primary/10 text-foreground"
                          : "border-border bg-card text-muted-foreground line-through hover:text-foreground",
                      ].join(" ")}
                    >
                      {on ? <CheckCircle2 className="h-3.5 w-3.5 text-primary" /> : <Circle className="h-3.5 w-3.5" />}
                      {a}
                    </button>
                  );
                })}
              </div>
              <p className="text-[11.5px] text-muted-foreground mt-2">
                {dropped.length ? `${dropped.length} field${dropped.length === 1 ? "" : "s"} excluded from the request.` : "All fields included in the request."}
              </p>
            </Card>

            <Card className="p-5">
              <SectionTitle hint={`${proposal.sources.length} sources`}>Recommended sources</SectionTitle>
              <ul className="space-y-2">
                {proposal.sources.map((s) => (
                  <li key={s.name} className="rounded-md border border-border px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="text-[13px] font-medium truncate">{s.name}</div>
                      <Badge tone={s.kind === "Company website" ? "success" : "info"}>{s.kind}</Badge>
                    </div>
                    <div className="text-[11.5px] text-muted-foreground mt-0.5">{s.note}</div>
                  </li>
                ))}
              </ul>
            </Card>

            <Card className="p-5">
              <SectionTitle>Solution metadata</SectionTitle>
              <div className="grid grid-cols-2 gap-2">
                {proposal.metadata.map((m) => (
                  <div key={m.label} className="rounded-md border border-border px-3 py-2">
                    <div className="text-[10.5px] uppercase tracking-wider text-muted-foreground font-semibold">{m.label}</div>
                    <div className="text-[12.5px] font-medium mt-0.5">{m.value}</div>
                  </div>
                ))}
              </div>
            </Card>
          </div>
        )}
      </div>
    </AppLayout>
  );
}

/* ----------------------------- chat question ----------------------------- */

function QuestionBubble({
  q,
  active,
  answers,
  otherDraft,
  onChoose,
  onOther,
  onConfirm,
  onSelectAll,
  onClearAll,
}: {
  q?: Question;
  active: boolean;
  answers: Answers;
  otherDraft: string;
  onChoose: (q: Question, id: string) => void;
  onOther: (q: Question, text: string) => void;
  onConfirm: (q: Question) => void;
  onSelectAll: (q: Question) => void;
  onClearAll: (q: Question) => void;
}) {
  if (!q) return null;
  const chosen = answers[q.id] ?? [];
  return (
    <div className="flex gap-2.5">
      <span className="h-7 w-7 shrink-0 rounded-md bg-secondary inline-flex items-center justify-center">
        <Bot className="h-3.5 w-3.5 text-muted-foreground" />
      </span>
      <div className="rounded-lg bg-secondary px-3.5 py-3 max-w-[85%] w-full">
        <div className="text-[13px] font-medium">
          {q.text}
          {q.multi && <span className="text-muted-foreground font-normal"> (pick as many as apply)</span>}
        </div>
        {q.help && <div className="text-[11.5px] text-muted-foreground mt-1">{q.help}</div>}

        {q.multi && active && (
          <div className="mt-2 flex items-center gap-1.5">
            <button
              onClick={() => onSelectAll(q)}
              className="rounded-md border border-border px-2 py-1 text-[11px] font-medium text-muted-foreground hover:text-foreground hover:border-primary/50"
            >
              Select all
            </button>
            <button
              onClick={() => onClearAll(q)}
              className="rounded-md border border-border px-2 py-1 text-[11px] font-medium text-muted-foreground hover:text-foreground hover:border-primary/50"
            >
              Clear all
            </button>
            <span className="text-[11px] text-muted-foreground">{chosen.length} selected — unselect what you don't need</span>
          </div>
        )}

        <div className="mt-2.5 grid grid-cols-1 sm:grid-cols-2 gap-1.5">
          {q.options.map((o) => {
            const on = chosen.includes(o.id);
            return (
              <button
                key={o.id}
                disabled={!active}
                onClick={() => onChoose(q, o.id)}
                className={[
                  "text-left rounded-md border px-3 py-2 text-[12.5px] transition",
                  on
                    ? "border-primary bg-primary/10 text-foreground"
                    : "border-border bg-card text-muted-foreground hover:border-primary/50 hover:text-foreground",
                  !active ? "opacity-60 cursor-default" : "",
                ].join(" ")}
              >
                <span className="font-medium">{o.label}</span>
                {o.hint && <span className="block text-[11px] text-muted-foreground mt-0.5">{o.hint}</span>}
              </button>
            );
          })}
        </div>

        {q.allowOther && active && (
          <div className="mt-1.5 flex items-center gap-2">
            <span
              className={[
                "shrink-0 rounded-md border px-2 py-1 text-[11px] font-medium",
                chosen.some(isOther) ? "border-primary bg-primary/10 text-foreground" : "border-border text-muted-foreground",
              ].join(" ")}
            >
              Other
            </span>
            <input
              value={otherDraft || chosen.filter(isOther).map(otherText)[0] || ""}
              onChange={(e) => onOther(q, e.target.value)}
              placeholder="None of these fit? Type your own…"
              className="flex-1 rounded-md border border-input bg-card px-2.5 py-1.5 text-[12.5px] outline-none focus:ring-2 focus:ring-ring/40 placeholder:text-muted-foreground"
            />
          </div>
        )}

        {active && (q.multi || chosen.some(isOther)) && (
          <Button size="sm" className="mt-2.5" disabled={!chosen.length} onClick={() => onConfirm(q)}>
            Continue <ArrowRight className="h-4 w-4" />
          </Button>
        )}
      </div>
    </div>
  );
}

/* ---------------------------- workflow diagram ---------------------------- */

const NODE_STYLE: Record<WorkflowNode["kind"], { cls: string; icon: typeof Bot }> = {
  io: { cls: "bg-emerald-500/15 text-emerald-400 border-emerald-500/40", icon: FileInput },
  fetch: { cls: "bg-sky-500/15 text-sky-400 border-sky-500/40", icon: Globe },
  llm: { cls: "bg-violet-500/15 text-violet-400 border-violet-500/40", icon: Bot },
  filter: { cls: "bg-amber-500/15 text-amber-500 border-amber-500/40", icon: Filter },
  merge: { cls: "bg-cyan-500/15 text-cyan-400 border-cyan-500/40", icon: Layers },
};

function WorkflowDiagram({ nodes }: { nodes: WorkflowNode[] }) {
  return (
    <div>
      <div className="flex flex-wrap items-stretch gap-x-1 gap-y-3">
        {nodes.map((n, i) => {
          const s = NODE_STYLE[n.kind];
          const Icon = n.id === "output" ? Download : n.id === "thirdparty" ? Boxes : s.icon;
          return (
            <div key={n.id} className="flex items-center">
              <div className="w-[104px] flex flex-col items-center text-center">
                <div className={`h-10 w-10 rounded-lg border inline-flex items-center justify-center ${s.cls}`}>
                  <Icon className="h-4.5 w-4.5" strokeWidth={1.75} />
                </div>
                <div className="mt-1.5 text-[10.5px] leading-tight text-muted-foreground">{n.label}</div>
              </div>
              {i < nodes.length - 1 && (
                <div className="flex items-center gap-0.5 -mt-5">
                  <span className="h-1 w-1 rounded-full bg-border" />
                  <span className="h-px w-4 bg-border" />
                  <span className="text-border text-[10px]">▶</span>
                </div>
              )}
            </div>
          );
        })}
      </div>
      <div className="mt-3 flex flex-wrap gap-2 text-[10.5px] text-muted-foreground">
        {[
          ["io", "Input / output"],
          ["fetch", "Fetch & discovery"],
          ["llm", "LLM step"],
          ["filter", "Filter / compare"],
          ["merge", "Merge"],
        ].map(([k, label]) => (
          <span key={k} className="inline-flex items-center gap-1.5">
            <span className={`h-2.5 w-2.5 rounded-sm border ${NODE_STYLE[k as WorkflowNode["kind"]].cls}`} /> {label}
          </span>
        ))}
      </div>
    </div>
  );
}

function Stat({ icon: Icon, label, value, sub }: { icon: typeof Database; label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-md border border-border px-3 py-2.5">
      <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
        <Icon className="h-3.5 w-3.5" /> {label}
      </div>
      <div className="text-[14px] font-semibold mt-1 leading-snug">{value}</div>
      {sub && <div className="text-[11px] text-muted-foreground mt-0.5 leading-snug">{sub}</div>}
    </div>
  );
}
