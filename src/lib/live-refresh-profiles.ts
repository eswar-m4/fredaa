...
import { getCustomLiveRefreshProfile } from "@/lib/custom-projects";
...
export function getLiveRefreshProfile(projectId: string): LiveRefreshProfile | null {
  // Hand-built profiles for onboarded real projects first, then the
  // dynamic per-customer profiles self-service Solutions launches create.
  return LIVE_REFRESH_PROFILES[projectId] ?? getCustomLiveRefreshProfile(projectId) ?? null;
}
