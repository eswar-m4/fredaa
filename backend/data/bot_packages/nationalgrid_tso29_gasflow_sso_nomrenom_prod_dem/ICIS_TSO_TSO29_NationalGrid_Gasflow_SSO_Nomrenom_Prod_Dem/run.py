...
ENTRYPOINT_SCRIPT = PACKAGE_ROOT / "ICIS_TSO_TSO29_NationalGrid_Gasflow_SSO_Nomrenom_Prod_Dem.py"
CONFIG_FILE = PACKAGE_ROOT / "Nationalgrid_config.json"
...
def _rewrite_config(config: Dict[str, Any], run_root: Path) -> Dict[str, Any]:
    ...
    rewritten["script_run_status"] = True
    rewritten["mailTrigger"] = False

    section_names = ("demand", "SSO", "gas-flow", "nomination", "renomination", "production")
    for section_name in section_names:
        section = rewritten.get(section_name)
        if not isinstance(section, dict):
            continue
        section["cache_path"] = _as_posix(run_root / "cache" / section_name / "<DATE>")
        section["output_path"] = _as_posix(run_root / "output" / section_name / "<DATE>")
...
def _load_config(package_copy: Path) -> Dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("NationalGrid config must be a JSON object")
    return payload
...
def run(context: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="nationalgrid_bot_", dir=str(workspace_root)) as temp_dir:
        ...
        if not records:
            raise RuntimeError("NationalGrid bot completed successfully but did not produce any parsed records")

        execution_metadata = {
            "bot_name": "NationalGrid TSO29 Gasflow SSO Nomrenom Prod Dem",
            "source": "https://data.nationalgas.com/reports/find-gas-reports/view",
